<template>
  <div>
    <b-navbar toggleable="lg" type="dark" variant="dark">
      <b-navbar-brand>
        <b-link class="navbar-brand" href="/transcribe">{{ $t('main.site_name') }}</b-link>
      </b-navbar-brand>
      <b-navbar-toggle target="nav-collapse"></b-navbar-toggle>
      <b-collapse id="nav-collapse" is-nav>
        <b-navbar-nav>
          <b-nav-item href="/transcribe" v-if="$store.state.auth.user">{{ $t('nav.start') }}</b-nav-item>

          <b-nav-item-dropdown :text="$t('nav.about')">
            <b-dropdown-item class="dropdown-item" href="/about/project">{{ $t('nav.project') }}</b-dropdown-item>
            <b-dropdown-item class="dropdown-item" href="/about/htr">{{ $t('nav.htr') }}</b-dropdown-item>
            <b-dropdown-item class="dropdown-item" href="/about/team">{{ $t('nav.team') }}</b-dropdown-item>
            <b-dropdown-item class="dropdown-item" href="/about/stats">{{ $t('nav.stats') }}</b-dropdown-item>
          </b-nav-item-dropdown>
        ...
        >
          <b-nav-item-dropdown>
            ...
            <b-dropdown-item
              ...
              :key="locale"
              :value="locale"
              @click="function(){setLanguage(locale)}.bind(this)"
            >
              <img class="flag" :src="require(`@/assets/images/flags/${locale}.png`)" alt />
          ...
          <b-nav-item-dropdown :text="$t('manuscripts')" v-if="$store.state.auth.user">
            <b-dropdown-item
              ...
              :key="i"
              :href="`/transcribe/${manuscript.name}`"
          ...
          <b-nav-item-dropdown>
            ...
            <b-dropdown-item v-if="isAnonymous">
              <b-link class="dropdown-item" :href="localePath('profile')">{{ $t('nav.profile') }}</b-link>
            </b-dropdown-item>
...
export default {
  ...
  methods: {
    ...
    goToNotificationContext(notification) {
      this.$router.push(`/conversation/${notification.href}`);
    ...
    getTranscribePath() {
      if (
        ...
        return `/transcribe/${this.$store.state.transcribe.manuscript.name}/${this.$store.state.transcribe.selected_line.page}/${this.$store.state.transcribe.selected_line.line}`;
      } else {
        return "/transcribe";
      }
    }
